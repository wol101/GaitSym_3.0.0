...
AMotorJoint::AMotorJoint(dWorldID worldID)
{
...
void AMotorJoint::SetStops(dReal low, dReal high)
{
...
void AMotorJoint::SetTargetVelocity(dReal targetVelocity)
{
...
void AMotorJoint::SetMaxTorque(dReal maximumTorque)
{
...
dReal AMotorJoint::GetAngleRate()
{
    dVector3 axis;
    dJointGetAMotorAxis(m_JointID, 0, axis); // get the world axis orientation

    dReal avel0 = dDOT(axis, dBodyGetAngularVel(dJointGetBody (m_JointID, 0)));
    dReal avel1 = dDOT(axis, dBodyGetAngularVel(dJointGetBody (m_JointID, 1)));

    dReal rate = avel1 - avel0; // I hope the sign is right here (it is different from dJointGetHingeAngleRate)
...
void AMotorJoint::SetAxis(dReal x, dReal y, dReal z, int axisMode)
{
    dVector3 result;
...
dReal AMotorJoint::GetAngle()
{
    dVector3 g;
    dJointGetAMotorAxis (m_JointID, 0, g); // this routine returns the axis in global coordinates

    const dReal *body1Rotation = dBodyGetRotation(dJointGetBody(m_JointID, 0));
    const dReal *body2Rotation = dBodyGetRotation(dJointGetBody(m_JointID, 1));
    ...
    dReal angleBody1 = atan2(a[1], a[0]);
    dMULTIPLY0_331(b, body2Rotation, p); // rotate by body rotation
    dMULTIPLY1_331(a, R, b); // rotate to axis coordinate system (using inverse of R)
    dReal angleBody2 = atan2(a[1], a[0]);

    dReal angle = angleBody2 - angleBody1;
...
void AMotorJoint::Dump()
{
    ...
    if (m_DumpStream)
    {
        dReal axisVelocity = GetAngleRate();
        dVector3 axis;
        dJointGetAMotorAxis(m_JointID, 0, axis); // get the world axis orientation
        dReal axisTorque = dDOT(axis, m_JointFeedback.t1);

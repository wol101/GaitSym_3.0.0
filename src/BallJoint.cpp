...
BallJoint::BallJoint(dWorldID worldID, int mode)
{
    // ball joint
...
void BallJoint::Attach(dBodyID body1, dBodyID body2)
{
    dJointAttach(m_JointID, body1, body2);
    dJointAttach(m_MotorJointID, body1, body2);
}
...
void BallJoint::SetAxes(dReal x0, dReal y0, dReal z0, dReal x1, dReal y1, dReal z1, dReal x2, dReal y2, dReal z2, int axisMode)
{
    dVector3 r;
...
void BallJoint::SetStops(dReal a0Low, dReal a0High, dReal a1Low, dReal a1High, dReal a2Low, dReal a2High)
{
...
void BallJoint::Dump()
{
    ...
    if (m_DumpStream)
    {
        dVector3 p;
        GetBallAnchor(p);
        dReal theta0 = dJointGetAMotorAngle(m_MotorJointID, 0);
        dReal theta1 = dJointGetAMotorAngle(m_MotorJointID, 1);
        dReal theta2 = dJointGetAMotorAngle(m_MotorJointID, 2);

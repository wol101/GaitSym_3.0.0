...
void HingeJoint::CalculateStopTorque()
{
    ...
    dReal t = 0; // calculated torque from error
    dReal ERP, CFM;
    dReal kp = 0;
    ...
    const dReal *bodyPosition = dBodyGetPosition(dJointGetBody(m_JointID, 0));
    pgd::Vector worldForceOffset(jointAnchor[0] - bodyPosition[0], jointAnchor[1] - bodyPosition[1], jointAnchor[2] - bodyPosition[2]);
    ...
    dReal torqueScalar = torqueJointAnchor.Magnitude();
    if (torqueScalar == 0)
    {
        m_axisTorque = 0;
        return;
...
void HingeJoint::SetStopTorqueWindow(int window)
{
    ...
    if (window > 1)
    {
        m_axisTorqueList = new dReal[window];
        memset(m_axisTorqueList, 0, sizeof(dReal) * window);

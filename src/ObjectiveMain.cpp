...
// static char *gInputKinematicsFilenamePtr = "/Users/wis/Synchronised/Research/Exploring Elasticity/Durham Paper/Exemplars/Composite/FastCompositeKinematics.txt";
    ...
    else
    {
        ...
        if (iPtr[0] == MPI_MESSAGE_ID_RELOAD_MODELCONFIG) // force a model reload
        {
            if (gModelConfigFile)
            {
                ...
                if (gDebug == MainDebug)
                {
                    *gDebugStream <<  "ReadModel loaded gModelConfigFile " << gModelConfigFile << "\n";
                }
            }
            std::cerr << "ReadModel loaded gModelConfigFile " << gModelConfigFile << "\n";
            delete [] data;
            return 1;
        }
...
int WriteModel()
{
    ...
    gTCP.StopClient();
#elif defined(USE_MPI)
    if (gConfigFilenamePtr) return(1); // MPI version used in filename mode so exit
